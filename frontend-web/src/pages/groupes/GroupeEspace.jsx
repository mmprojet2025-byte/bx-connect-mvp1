            ...
            {groupe.description && (
              <section className="mb-5 rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
                <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wide text-slate-500">
                  <AppIcon name="BookOpen" className="h-4 w-4 text-blue-700" />
                  {t('groups.presentation')}
                </h2>
                <p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-600">{groupe.description}</p>
              </section>
            )}
...
function DiscussionPanel({ isMember, t }) {
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
      ...
      <Link to={isMember ? '/messagerie' : '/groupes'} className={`inline-flex h-fit items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-black ${
...
function LinkedItemsPanel({ type, items, language, t }) {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      {items.map(item => (
        <Link key={item.id} to={isActivity ? `/activites/${item.id}` : `/projets/${item.id}`} className="rounded-lg border border-slate-100 bg-slate-50 p-4 transition hover:bg-white hover:shadow-md">
