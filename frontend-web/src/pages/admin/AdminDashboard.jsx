...
export default function AdminDashboard() {
  ...
  if (loading) return <p style={{ padding: '2rem' }}>{t('admin.loading')}</p>;

  return (
    <div style={{ maxWidth: '1000px', margin: '2rem auto', padding: '0 1rem' }}>

      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ color: '#1A3C5E', margin: 0 }}>🛡️ {t('admin.title')}</h1>
        <p style={{ color: '#4A6A8A', marginTop: '0.5rem' }}>{t('admin.subtitle')}</p>
      </div>
      ...
      <h2 style={{ color: '#1A3C5E', marginBottom: '1rem' }}>{t('admin.manage')}</h2>
...
function StatCard({ label, value, color, icon }) {
  return (
    <div style={{ background: '#fff', borderRadius: '12px', padding: '1.25rem', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', borderLeft: `4px solid ${color}` }}>
...
function NavCard({ to, icon, title, description, color }) {
  return (
    <Link to={to} style={{ textDecoration: 'none' }}>
      <div
        style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', cursor: 'pointer', transition: 'transform 0.15s', borderTop: `3px solid ${color}` }}
