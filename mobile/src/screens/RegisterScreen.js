...
function LegalStep({ accepted, onToggle, onTerms, onPrivacy, onContinue, t }) {
  return (
    <View>
      <View style={styles.discoveryTags} accessibilityRole="list">
        {['activities', 'groups', 'projects', 'community'].map((item, index) => (
          <View
            key={item}
            style={[styles.discoveryTag, styles[`discoveryTag${index + 1}`]]}
            accessibilityRole="listitem"
          >
            <Text style={styles.discoveryTagText}>{t(`registerWizard.step1.features.${item}`)}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={styles.googleBlock}>
        ...
        <Text style={styles.googleSoon}>{t('registerWizard.google.soon')}</Text>
      </View>

      <View style={styles.legalDocuments}>
        <DocumentLink label={t('legal.links.terms')} onPress={onTerms} />
        <DocumentLink label={t('legal.links.privacy')} onPress={onPrivacy} />
      </View>
      ...
      <TouchableOpacity
        ...
        accessibilityLabel={t('legal.acceptanceRequired')}
      >
        <View style={[styles.checkbox, accepted && styles.checkboxChecked]}>
          {accepted && <AppIcon name="check" size={16} color="#fff" />}
        </View>
        <Text style={styles.acceptanceText}>{t('registerWizard.acceptance')}</Text>
      </TouchableOpacity>
      <PrimaryButton label={t('registerWizard.continue')} onPress={onContinue} disabled={!accepted} />
    </View>
  );
}

function DocumentLink({ label, onPress }) {
  return (
    <TouchableOpacity style={styles.documentLink} onPress={onPress} accessibilityRole="link" accessibilityLabel={label}>
      <AppIcon name="document-text-outline" size={20} color={COLORS.bxBlueLight} />
...
function IdentityStep({ form, update, focusedField, setFocusedField, refs, onContinue, t }) {
  return (
    <View>
      ...
      />
      <PrimaryButton label={t('registerWizard.continue')} onPress={onContinue} />
    </View>
  );
}

function FormField({ inputRef, name, label, focusedField, setFocusedField, ...inputProps }) {
  return (
...
function PrimaryButton({ label, onPress, disabled = false, loading = false }) {
  ...
  return (
    <TouchableOpacity
      style={[styles.primaryButton, unavailable && styles.primaryButtonDisabled]}
      onPress={onPress}
...
const styles = StyleSheet.create({
  ...
  progressBlock: { width: '100%', maxWidth: 540, alignSelf: 'center', marginBottom: 14 },
  progressText: { color: COLORS.bxBlue, fontSize: 13, lineHeight: 18, fontWeight: '800', marginBottom: 8 },
  progressTrack: { flexDirection: 'row', gap: 6 },
  progressSegment: { flex: 1, height: 4, borderRadius: 2, backgroundColor: '#CBD5E1' },
  progressSegmentActive: { backgroundColor: COLORS.impactOrange },
  card: { width: '100%', maxWidth: 540, alignSelf: 'center', backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, borderRadius: 18, padding: 18 },
  title: { color: COLORS.text, fontSize: 24, lineHeight: 31, fontWeight: '900' },
  ...
  discoveryTags: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginBottom: 16 },
  discoveryTag: { borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6 },
  discoveryTag1: { backgroundColor: COLORS.softBlue },
  discoveryTag2: { backgroundColor: '#FFF3E8' },
  discoveryTag3: { backgroundColor: '#E8F7F2' },
  discoveryTag4: { backgroundColor: '#F2EDFF' },
  discoveryTagText: { color: COLORS.text, fontSize: 12, lineHeight: 17, fontWeight: '800' },
  googleBlock: { alignItems: 'center', marginBottom: 14 },
  googleButtonDisabled: { width: '100%', minHeight: 48, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#CBD5E1', borderRadius: 14, backgroundColor: '#F1F5F9', paddingHorizontal: 16 },
  googleButtonText: { color: '#64748B', fontSize: 14, lineHeight: 20, fontWeight: '800' },
  googleSoon: { color: COLORS.muted, fontSize: 11, lineHeight: 16, marginTop: 5 },
  legalDocuments: { gap: 8 },
  documentLink: { minHeight: 50, flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderColor: COLORS.border, borderRadius: 13, paddingHorizontal: 13, backgroundColor: COLORS.softBlue },
  documentLinkText: { flex: 1, color: COLORS.bxBlue, fontSize: 14, lineHeight: 20, fontWeight: '800' },
  legalVersion: { color: COLORS.muted, fontSize: 11, lineHeight: 16, marginTop: 10 },
  acceptanceRow: { minHeight: 56, flexDirection: 'row', alignItems: 'center', gap: 11, marginVertical: 15, paddingVertical: 6 },
  checkbox: { width: 26, height: 26, borderRadius: 7, borderWidth: 2, borderColor: COLORS.bxBlueLight, alignItems: 'center', justifyContent: 'center' },
  ...
  inputShell: { minHeight: 50, flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: '#CBD5E1', borderRadius: 14, backgroundColor: '#F8FAFC', paddingHorizontal: 13 },
  ...
  passwordHelp: { backgroundColor: COLORS.page, borderRadius: 13, padding: 11, marginTop: -7, marginBottom: 15, gap: 4 },
  ...
  primaryButton: { minHeight: 50, alignItems: 'center', justifyContent: 'center', borderRadius: 14, backgroundColor: COLORS.impactOrange, paddingHorizontal: 18, marginTop: 4 },
